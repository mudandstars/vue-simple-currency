<script setup lang="ts">
	...
	watch(
		...
		async () => {
			...
			if (addedZero(localValue.value)) {
				localValue.value =
					localValue.value.substring(
						0,
						localValue.value.indexOf(",")
					) + "0,00€";
	...
	const enteredNumber = (value: string) =>
		!isNaN(parseInt(value[value.length - 1])) &&
		value[value.length - 1] !== "0";
	const deletedComma = (value: string) =>
	...
	const displayValue = () => {
		let clearedValue = localValue.value.slice(
			0,
			localValue.value.indexOf(",")
		);
